...
function admin_user() {
  ...
  for (var i = 1; i < userarray.length; i++) {
    msg +=
      ...
      i +
      '" ></i>' +
      '</div><button type="button" class="btn_xoaUser" onclick="xacNhan_xoaUser(' +
      i +
      ')">Xóa user</button></div>';
  }
  console.log(msg);
...
function del_user() {
  var userarray = JSON.parse(localStorage.getItem("user"));
  userarray.splice(index_user, 1);
  localStorage.setItem("user", JSON.stringify(userarray));
  admin_user();
  cancel();
}
function xacNhan_xoaUser(index) {
  ...
  a.innerHTML =
    ...
    <div style="padding:10px; float:left;width:40%"><button class="btn_confirm_xoa_sp" id="yes" onclick="del_user()">Yes</button></div>\n\
  </div>';
  index_user = index;
}
...
function inner_sua_sp(i) {
  ...
  msg =
    ...
    productarray[i].product_name +
    '" ></div>\n\
      <div id="div_sua_img"><img id="img_preview_sp" alt="hello thầy" style="color:black"  src="' +
    productarray[i].product_img +
    '">\n\
      <label >Thêm ảnh\n\
