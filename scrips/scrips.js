...
function default_product() {
  if (localStorage.getItem("product") == null) {
    ...
    var product14 = {
      product_name:
        "Giày Air Jordan 11 Retro Low IE Light Orewood Brown 919712-102",
      product_img:
...
function taoDonHang() {
  var userarray = JSON.parse(localStorage.getItem("user"));
  var giohangarray = JSON.parse(localStorage.getItem("GioHang"));
...
function inner_donhang() {
  ...
  for (var i = 0; i < DonHangArray.length; i++)
    if (userarray[login_index].email == DonHangArray[i].email) {
      msg +=
      '<div class="order_block" onclick="inner_chitiet_donhang('+i+')">\n\
        <div class="div_noidung_order" > \n\
          <div id="order_name">Tên: '+DonHangArray[i].hoten+'</div>\n\
          <div id="order_sdt">Phone: '+DonHangArray[i].sdt+'</div>';
          if(DonHangArray[i].xuli == "true")
          msg += '<div id="order_xuli">Trạng thái: Đã xử lí</div>\n';
        else
          msg += '<div id="order_xuli">Trạng thái: Chưa xử lí</div>\n';

      msg += 
      '<div id="order_email">Email: '+DonHangArray[i].email+' </div>\n\
      <div id="order_diachi">Địa chỉ: '+DonHangArray[i].diachi+' </div>\n\<div id="order_tong">Tổng: '+DonHangArray[i].tongtiendonhang+' </div>\n\
      <div id="order_thanhtoan">Phương thức thanh toán: '+DonHangArray[i].thanhtoan+'</div>\n\
      </div>\n\
  ...
  if(msg == "")
    document.getElementById("form_donhang").style.display = "none";
  else{
    document.getElementById("form_donhang").style.display = "block";
    document.getElementById("donhang_container").innerHTML = msg;
  }
}
function inner_chitiet_donhang(num){
  var DonHangArray = JSON.parse(localStorage.getItem("DonHang"));
  var sp = document.getElementById("order_sp"+num);
  var msg ="";
  var a = sp.innerHTML;
  if( a == ""){
    for(var i=0; i<DonHangArray[num].giohang.length; i++){
      msg +=`
      <div class="item-gio-hang">
        <div class="hinhAnh">
          <img src=${DonHangArray[num].giohang[i].product_img} />
        </div>
        <p class="ten">${DonHangArray[num].giohang[i].product_name}</p>
        <div class="gia">
          <span class="giagoc" id="gia">${DonHangArray[num].giohang[i].product_price}</span>
        </div>
        <p class="soluong" >${DonHangArray[num].giohang[i].soluong}  </p>
        <p class="tongtien" id="tongtien">
          ${(parseInt(DonHangArray[num].giohang[i].product_price.replaceAll(".", "").replace("đ", "")) * DonHangArray[num].giohang[i].soluong).toLocaleString() +" đ"}
        </p>
      </div>`;
    }
  }
